...
strawberry <- read_csv("/Users/zhangxingchen/haha/MSSP/Strawberry/strawberry.csv", col_names = TRUE)
...
```{r}
#| echo: false
strwb_census=strawberry[strawberry$Program=='CENSUS',]
strwb_survey=strawberry[strawberry$Program=='SURVEY',]
```
...
title ="Organic Strawberry Sales ($) in 2021")
```
From these two graphs, we can see that California is leading in organic strawberry sales. What about the strawberries that are not organic?

```{r}
#| echo: false
strawberry %>%
 filter(Year > 2020L & Year < 2022L) %>%
 filter(!(Domain %in% c("ORGANIC STATUS", "TOTAL"
))) %>%
 ggplot() +
  aes(x = State) +
  geom_bar(fill = "black") +
  labs(
    x = "States",
    y = "Counts",
    title = "Strawberry Sales That Are not Organic in 2021."
  ) +
  theme_minimal()
```
From this graph, we can see that when it comes to strawberries that are not organic, California is still leading. There is the possibility that some confounders may exist, such as census and survey.
...
strawberry %>%
 ...
 ggplot() +
  ...
  labs(
    ...
    y = "Counts",
    title = "Strawberry Sales That Are Not Organic in 2021.",
    subtitle = "Survey and Census"
  ) +
  theme_minimal()
```

From this graph, we can see clearly that there is only survey data. So the results are not completely reliable since there may exist sample errors. 

```{r}
gdp=read.csv("/Users/zhangxingchen/haha/MSSP/Strawberry/gdp.csv")
gdp_21=gdp[2,]
max=which.max(gdp_21)
max
```

Source: https://usafacts.org/metrics/gross-domestic-product-gdp-by-state/?adjustment=None&timeGranularity=Yearly

For this part, I am intending to find if there exists the relationship between GDP and strawberry sales in California. I found the data of 2021 USA GDP from the internet and found that California actually had the highest GDP in 2021. So GDP may be one of the factors that impacts the strawberry sales.
